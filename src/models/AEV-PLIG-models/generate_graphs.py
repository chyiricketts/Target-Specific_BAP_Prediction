...
# helper function for build_graphs
def load_ligand(ligand_path):
    if ligand_path.endswith(".sdf"):
        ...
        raise ValueError(f"Unknown ligand format: {ligand_path}")
...
def build_graphs(df):
    ...
    atom_keys = pd.read_csv(
        os.path.join(BASE_DIR, "data/files", "PDB_Atom_Keys.csv"),
        sep=","
    )

    atom_map = pd.DataFrame(pd.unique(atom_keys["ATOM_TYPE"]))
    atom_map[1] = np.arange(len(atom_map)) + 1
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if mol is None:
            print("can't read molecule structure:", unique_id)
            failed.append(unique_id)
            continue

        try:
            mol_df, aevs = GetMolAEVs_extended(
                protein_path, mol, atom_keys, radial_coefs, atom_map
            )
...
if __name__ == "__main__":
    ...
    # dump the arguments into jason
    exp_dir = os.path.join(BASE_DIR, "scripts", args.exp_name)
    os.makedirs(os.path.join(exp_dir, "json"), exist_ok=True)
    with open(os.path.join(exp_dir, "json", "config.json"), "w") as f:
    ...
    # save some stats
    node_sizes = [g[0] for g in all_graphs.values()]  # first element = num nodes

    stats = {
        "n_graphs": len(all_graphs),
        "avg_nodes_per_graph": float(np.mean(node_sizes)),
        "min_nodes_per_graph": int(np.min(node_sizes)),
        "max_nodes_per_graph": int(np.max(node_sizes)),
    }

    stats_path = os.path.join(exp_dir, "json", "graph_stats.json")
    with open(os.path.join(stats_path), "w") as f:
        json.dump(stats, f, indent=2)
    print(f"Saved graph stats to {stats_path}")