...
# needs to be run from ts_AEV-PLIG for access to the helper function files
...
def process_data(config):
    ...
    df = pd.read_csv(os.path.join(BASE_DIR, "data/processed/fep_prediction_info.csv"))
    ...
    if not config.skip_validation:
        ...
        atom_keys = pd.read_csv(os.path.join(BASE_DIR, "data/files", "PDB_Atom_Keys.csv"), sep=",")
    ...
    new_dataset_csv = "fep_prediction_info_processed.csv"
...
def generate_graphs(config):
    ...
    output_file_graphs = os.path.join(BASE_DIR, "data", "processed", "fep_graphs.pickle")
...
if __name__ == "__main__":    
    ...
    # to remove those that cannot be parsed-- for my dataset, none are removed
    process_data(config) 
